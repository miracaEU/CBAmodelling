...
def calculate_growth_rate_factor(growth_rate=3.0,
    ...
    if isinstance(growth_rate, float):
        ...
        for i, (year,rate) in enumerate(growth_rate):
            if year > start_year:
                growth_rates.append((year,prod([1 + v[1]/100.0 for v in growth_rate[:i+1]])))
            else:
                growth_rates.append((year,1))  

    return growth_rates

def calculate_discounting_rate_factor(discount_rate=4.0,
                                start_year=2025,end_year=2100,
                                maintain_period=5.0,skip_year_one=False):
...
def estimate_risk_time_series(
    ...
    risk_dataframe[risk_scenario_columns] = risk_dataframe[risk_scenario_columns].astype(str)
    scenarios = risk_dataframe[risk_scenario_columns].drop_duplicates(keep="first")
    
    index_columns = [c for c in risk_dataframe.columns.values.tolist() if c not in risk_value_columns]
    ...
    for sc in scenarios.itertuples():
        ...
        for rv in risk_value_columns:
            ...
            df_t = (df_t.set_index(idx_cols).pivot(
                                columns=year_column
                                    )[rv].reset_index().rename_axis(None, axis=1)).fillna(0)
            series = np.array([list(timeseries)*len(df_t.index)]).reshape(len(df_t.index),len(timeseries))
            df_t[series[0]] = interp1d(years,df_t[years],fill_value="extrapolate",bounds_error=False)(series[0])
...
def estimate_adaptation_cost_time_series(
    ...
    timeseries = np.arange(start_year,end_year+1,1)
    cost_dataframe[cost_scenario_columns] = cost_dataframe[cost_scenario_columns].astype(str)
    scenarios = cost_dataframe[cost_scenario_columns].drop_duplicates(keep="first")
    
    cost_columns = [initial_investment_column] + periodic_investment_columns + recurrent_investment_columns
    cost_time_series = []
    for sc in scenarios.itertuples():
        query_values = []
        ...
        ini_cost_series = [df[initial_investment_column].values[0]] + [0.0]*(len(timeseries)-1)
        if len(periodic_investment_columns) > 0:
            ...
            maintain_years = calculate_discounting_rate_factor(
                                                    ...
                                                    maintain_period=maintain_period,skip_year_one=True)
            per_cost_series = list((df[periodic_investment_columns[0]].values[0])*maintain_years)
        else:
        ...
        if len(recurrent_investment_columns) > 0:
            ...
            maintain_years = calculate_discounting_rate_factor(
                                                    ...
                                                    start_year=start_year,end_year=end_year,
                                                    maintain_period=maintain_period,skip_year_one=True)
            rec_cost_series = list((df[recurrent_investment_columns[0]].values[0])*maintain_years)
        else:
            rec_cost_series = [0.0]*len(timeseries)

        cost_time_series.append(tuple([
